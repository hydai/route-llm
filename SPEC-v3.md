...
- **`crates/core/src/model.rs`（改，加法）**：新增 `BudgetBreakdown`、`ReasonCode`/`Dimensions`；`Recommendation` 加選用 `budget`；`RoutingPreferences` 加選用 `policy: Policy`（預設 `Balanced`）。
- **`crates/core/src/lib.rs`（改）**：`pub mod budget;`、re-export `BudgetRouter`、`Policy`、`BudgetBreakdown`。
- **`crates/server/src/dto.rs`（改，加法）**：`PrefsInput` 加選用 `policy`（`#[serde(default)]`）。responses 沿用 `Recommendation` 序列化，budget 自動隨附。
- **`crates/server/src/main.rs`（改）**：`choose_router` 加 `"budget" => Box::new(BudgetRouter::new())`。
...
`RoutingPreferences` 加選用 `policy`（預設 `Balanced`；DTO `#[serde(default)]`，向後相容）：
...
- 推論路徑加法：新增策略不改既有兩者；`Recommendation` 新欄位為 `Option` 且 skip-if-none。
...
inference 時呼叫 LLM、服務代執行 verify/fallback/工具、改 v1/v2 推論核心、新 API 端點、per-dimension 人工 gold（v3.1）、動 v2.2 凍結資料、把 policy 與 cost_bias 以外的旋鈕暴露到 API、自動重出貨或自動改預設 router。
...
- 子指令命名（`fit-budget`/`eval-budget`/`label --dims`/`crosseval --dims`）與旗標於實作定稿。
